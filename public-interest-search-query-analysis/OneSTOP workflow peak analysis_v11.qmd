...
species_description_plots <- list.files("data/species_descriptions/", full.names = T) %>% 
  map(function(species_description_path){
    ...
    species_data <- europe_mainland %>% 
      ...
      inner_join(species1_description, by = "name") %>% 
      mutate(number_of_anomalies = factor(number_of_anomalies, levels = 0:4))
    
    # plot
    ggplot(species_data) +
      geom_sf(aes(fill = number_of_anomalies), color = "grey90", size = 0.8) +
      geom_sf(
